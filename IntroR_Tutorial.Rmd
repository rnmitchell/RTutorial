...
What do these symbols mean?? Some may are obvious (`>` and `<` mean `greater than` and `less than`).

But why is `equals to` operator a double equals sign (`==`)?? 

Remember that what the `=` operator means in R? It sets the name of the dataset, variable, vector, function, etc.! Therefore, the `==` is the operator to mean `equal to`.


The `>=` and `<=` means `greater than or equal to` and `less than or equal to`.


These operators can also be used to *remove* rows containing a specific column value using the `!=`:

```{r subset_remove, exercise=TRUE}
new_df = subset(mtcars, cyl!=6)
new_df
```
This means the `mtcars` dataframe is subsetted for all values of the `cyl` column that do NOT equal 6!
...
Finally, in the code chunk below, filter the `iris` data frame to rows with `Sepal.Length` less than 4 _or_ `Petal.Length` is greater than 3. (HINT: the command for `or` is `|`)
...
```{r ggplot_quiz}
quiz(
  question("In the above code, the ggtitle() command contains `\\n`. What does this symbol do?",
    answer("Nothing! It's a typo."),
    answer("It makes the text bold."),
    answer("It makes the text italic."),
    answer("It acts as a line break, moving the text following it to the next line.", correct=TRUE)
  ))
```
